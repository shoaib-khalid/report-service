package com.kalsym.report.service;
            ...
            map.forEach((RequestMappingInfo requestMappingInfo, HandlerMethod handlerMethod) -> {
                try {
                    if (!handlerMethod.getMethod().getName().equalsIgnoreCase("error")
                        ...
                        Logger.application.info(Logger.pattern, ReportServiceApplication.VERSION, "", "name: " + requestMappingInfo.getName());
                        Logger.application.info(Logger.pattern, ReportServiceApplication.VERSION, "", "method: " + handlerMethod.getMethod().getName());
                        Logger.application.info(Logger.pattern, ReportServiceApplication.VERSION, "", "description: " + requestMappingInfo.toString());
                        ...
                        Logger.application.info(Logger.pattern, ReportServiceApplication.VERSION, "", "inserted authority", "");

                    }

                } catch (Exception e) {
                    Logger.application.warn(Logger.pattern, ReportServiceApplication.VERSION, "error inserting authority", e.getMessage());
                }

            });

            ResponseEntity<String> response = restTemplate.postForEntity(userServiceUrl, authorities, String.class);
            Logger.application.info(Logger.pattern, ReportServiceApplication.VERSION, "response: " + response);
