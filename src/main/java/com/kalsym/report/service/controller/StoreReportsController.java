...
//    public ResponseEntity<HttpResponse> salesWeeks(HttpServletRequest request, @RequestParam(required = false, defaultValue = "") int startWeekNo, @RequestParam(required = false, defaultValue = "") int endWeekNo, @PathVariable("storeId") String storeId,
//                                                   @RequestParam(defaultValue = "created", required = false) String sortBy,
//                                                   @RequestParam(defaultValue = "ASC", required = false) String sortingOrder) throws Exception {
//        HttpResponse response = new HttpResponse(request.getRequestURI());
//        Date date = new Date();
...
//
//            List<Object[]> orders = orderRepository.findAllByStoreIdAndDateRangeAndPaymentStatus(storeId, stDate, endDate, "Paid", sortBy, sortingOrder);
//
//            for (int k = 0; k < orders.size(); k++) {
//                Response.Sales sale = new Response.Sales();
//                sale.setStoreId(orders.get(k)[1].toString());
